...
def extract_diseases(tokens, labels):
    diseases = []
    current_disease = []
    ...
    for token, label in zip(tokens, labels):
        ...
        elif label.startswith('I-') and current_disease:
            current_disease.append(token)
        else:
            if current_disease:
                diseases.append(' '.join(current_disease))
...
def handle_request(data):
    ...
    diseases = extract_diseases(cleaned_tokens, cleaned_labels)
...
def predict_pdf():
    try:
        ...
        diseases = handle_request(pdf_text)
